...
function readInitialState(): ArtworkState {
  ...
  const numberParam = (name: string, fallback: number) => { const value = Number(params.get(name)); return Number.isFinite(value) ? value : fallback }
  return { seed: Math.floor(numberParam('seed', initialState.seed)), palette: palette && palette in palettes ? palette : initialState.palette, worldType: worldTypes.includes(type) ? type : initialState.worldType, density: clamp(numberParam('density', initialState.density)), drift: clamp(numberParam('drift', initialState.drift)), glow: clamp(numberParam('glow', initialState.glow)) }
}
...
function App() {
  ...
  const draw = useCallback((target: HTMLCanvasElement, renderState: ArtworkState, output?: { width: number; height: number }, time = 0) => {
    ...
    const count = Math.round(20 + renderState.density * 1.45); const isOrbit = renderState.worldType === 'orbit'; const isConfetti = renderState.worldType === 'confetti'; const motion = time * 0.00035
    for (let i = 0; i < count; i += 1) {
      const angle = random() * Math.PI * 2; const distance = 50 + random() * 285; const phase = random() * Math.PI * 2
      const x = isConfetti ? (random() - 0.5) * 820 : Math.cos(angle + (isOrbit ? motion * (1 + i % 3 * 0.25) : 0)) * distance + Math.sin(i * 0.7 + phase + motion) * renderState.drift
      const y = isConfetti ? (random() - 0.5) * 520 : Math.sin(angle + (isOrbit ? motion * (1 + i % 3 * 0.25) : 0)) * distance * (isOrbit ? 0.9 : 0.62) + Math.cos(i * 0.43 + phase + motion) * renderState.drift * 0.35
      const size = 8 + random() * (18 + renderState.drift * 0.42); const color = palette.colors[Math.floor(random() * palette.colors.length)]
      context.save(); context.translate(x, y); context.rotate(angle + random() * 0.8 + (isConfetti ? motion * (i % 2 ? 1 : -1) : 0)); context.globalAlpha = 0.35 + random() * 0.55; context.shadowColor = color; context.shadowBlur = renderState.glow * 0.7; context.fillStyle = color; context.beginPath()
      if (isConfetti) { context.fillRect(-size * 0.7, -size * 0.7, size * 1.4, size * 1.4); context.strokeStyle = color; context.lineWidth = 1.5; context.beginPath(); context.arc(size * 1.1, -size * 0.8, size * 0.45, 0, Math.PI * 1.5); context.stroke() }
    ...
    context.globalAlpha = 0.9; context.shadowBlur = 0
    if (isOrbit) {
      context.fillStyle = palette.colors[1]; context.shadowColor = palette.colors[1]; context.shadowBlur = renderState.glow * 1.4; context.beginPath(); context.arc(0, 0, 38 + renderState.drift * 0.12, 0, Math.PI * 2); context.fill()
      context.shadowBlur = 0; context.fillStyle = palette.sky; context.globalAlpha = 0.38; context.beginPath(); context.arc(-12, -9, 10, 0, Math.PI * 2); context.fill(); context.globalAlpha = 0.9
    } else if (!isConfetti) {
      context.strokeStyle = palette.colors[0]; context.lineWidth = 2; context.globalAlpha = 0.7
      for (let i = 0; i < 7; i += 1) { const stem = 42 + i * 13; context.beginPath(); context.moveTo(0, 20); context.quadraticCurveTo(Math.sin(i + motion) * (18 + renderState.drift * 0.2), -stem * 0.45, Math.cos(i * 1.7 + motion) * stem, -stem); context.stroke() }
      context.globalAlpha = 1; context.fillStyle = palette.colors[1]; context.shadowColor = palette.colors[1]; context.shadowBlur = renderState.glow; context.beginPath(); context.arc(0, 20, 25, 0, Math.PI * 2); context.fill()
    }
    context.shadowBlur = 0; context.strokeStyle = palette.colors[0]; context.lineWidth = 2; const rings = isConfetti ? 2 : isOrbit ? 11 : 4
    for (let i = 0; i < rings; i += 1) { context.beginPath(); context.arc(0, 0, 70 + i * 35 + random() * 12, random() * 0.8 + (isOrbit ? motion * (i % 2 ? 1 : -1) : 0), Math.PI * (1.1 + random() * 0.6)); context.stroke() }
    context.restore()
  }, [])
  ...
  useEffect(() => {
    ...
    let frame = 0
    const render = (time: number) => { draw(canvas, state, undefined, time); frame = window.requestAnimationFrame(render) }
    frame = window.requestAnimationFrame(render)
    const resize = () => draw(canvas, state, undefined, performance.now())
  ...
  const copyLink = async () => {
    try {
      ...
      else { const field = document.createElement('textarea'); field.value = window.location.href; field.setAttribute('readonly', ''); field.style.position = 'fixed'; field.style.opacity = '0'; document.body.appendChild(field); field.select(); document.execCommand('copy'); field.remove() }
      setCopied(true); window.setTimeout(() => setCopied(false), 1500)
    } catch { window.prompt('Copy this world link', window.location.href) }
  }
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <aside className="controls"><div><p className="kicker">make a tiny world</p><h1>Something small,<br /><em>strangely alive.</em></h1><p className="intro">Shape a little universe from a handful of gentle rules. There are no wrong turns.</p></div>
        <div className="control-group"><div className="control-heading"><span>World type</span><span className="control-value">{state.worldType}</span></div><div className="type-row">{worldTypes.map((type) => <button key={type} className={`type-button ${state.worldType === type ? 'selected' : ''}`} aria-pressed={state.worldType === type} onClick={() => update('worldType', type)}>{type}</button>)}</div></div>
...
function Slider({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) { return <label className="slider"><span className="control-heading"><span>{label}</span><span className="control-value">{value}</span></span><input aria-label={label} type="range" min="0" max="100" value={value} onChange={(event) => onChange(Number(event.target.value))} /></label> }
